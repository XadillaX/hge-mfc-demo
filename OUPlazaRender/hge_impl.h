...
class HGE_Impl : public HGE
{
    /** XadillaX+ */
private:
    ::CRITICAL_SECTION          m_csInputQueue;

public:
    void InitInputQueueCS()     { ::InitializeCriticalSection(&m_csInputQueue); }
    void ReleaseInputQueueCS()  { ::DeleteCriticalSection(&m_csInputQueue); }
    void LockInputQueueCS()     { ::EnterCriticalSection(&m_csInputQueue); }
    void UnlockInputQueueCS()   { ::LeaveCriticalSection(&m_csInputQueue); }
...
public:
    ...
    /** XadillaX+ */
    virtual void        CALL    AttachWindowsProc(WNDPROC proc)
    {
        m_pAttachProc = proc;
    }
    WNDPROC             CALL    GetAttachedWindowsProc()
    {
        return m_pAttachProc;
    }
